...
namespace mirazabal
{
	namespace sio
	{
		websocket_endpoint::websocket_endpoint() : m_next_id(0), m_connected(false)
		{
			...
			m_endpoint.set_message_handler([&](websocketpp::connection_hdl con, websocketpp::client<websocketpp::config::debug_asio>::message_ptr msg)
			{
		...
		int websocket_endpoint::connect(std::string const & uri)
		{
			try
			{
				...
				m_network_future = std::async(std::launch::async, [&]()
				{
					...
					{
						std::cout << "Exception not catch" << std::endl;
						throw std::runtime_error("Exception not catched from the network future");
					}
		...
		void websocket_endpoint::emit(std::string const& msg_name, std::string const&  msg)
		{
			...
			m_packet_mgr.encode(p); // this will fire the event send, pedazo de mierda.... 
		...
		{
			if (message && message->get_flag() == sio::sio_message::flag_object)
			{
				...
				if (it != values->end() && it->second->get_flag() == sio::sio_message::flag_integer) {
